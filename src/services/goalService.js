...
export async function listGoals(spaceId = null) {
  ...
  if (!userId) return []
  const query = supabase.from(table).select('*').eq('space_id', spaceId).order('target_date', { ascending: true })
...
export async function saveGoal(goal, spaceId = null) {
  ...
  const { id, ...rest } = goal
  const payload = { ...rest, space_id: effectiveSpaceId }
  if (id) {
  ...
  const { data, error } = await supabase
    .from(table).insert({ ...payload, user_id: userId }).select().single()
  if (error) throw error
  return data
}
